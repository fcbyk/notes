...
import { useData,useRouter} from 'vitepress' // useData函数是vp中用于获取页面数据的钩子函数
...
export default Object.assign({}, Theme, {
  ...
  Layout: () => {
    ...
    console.log(routerInstance.route)

    routerInstance.onBeforeRouteChange = (to) => {
      if (to === '/resume') {
        if(sessionStorage.getItem('isAuthenticated') == 'true') return true
        
        routerInstance.go('/resume-password')
        return false
      }
